...
class ShortTermAnalysisTask(Base):
    __tablename__ = "short_term_analysis_tasks"
...
class ShortTermShareLink(Base):
    __tablename__ = "short_term_share_links"

    code: Mapped[str] = mapped_column(String(16), primary_key=True)
    task_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("short_term_analysis_tasks.id"), nullable=False
    )
    expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    visit_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    created_at: Mapped[datetime] = mapped_column(
